...
En el sidebar hay 3 workflows de ejemplo:
- **Workflow 1 (API)** → consulta a una API HTTP y procesa la respuesta.
- **Workflow 2 (ETL)** → extrae, transforma y carga datos.
- **Workflow 3 (HTTP)** → prueba de peticiones HTTP con condiciones.

Son útiles para explorar las capacidades de MiniFlow sin construir desde cero.
